...
//    }
    ...
    public static Invoice calculateAndCreateInvoice(List<InvoiceRecord> invoiceRecords, String sellerName, List<Material> materials) {
        ...
        for (Map.Entry<MaterialType, List<InvoiceRecord>> entry : invoiceRecordsByMaterialType.entrySet()) {
            MaterialType materialType = entry.getKey();
            List<InvoiceRecord> recordsForMaterial = entry.getValue();
            System.out.println("Records for material = " + recordsForMaterial.size());
            Material material = findMaterialByMaterialType(materialType, materials);

            int totalAmountToPayForMaterial = calculateAmountToPayForMaterial(recordsForMaterial, material);

            //Third step. I add the material description and amount to pay for that material to the list
    ...
    private static int calculateAmountToPayForMaterial(List<InvoiceRecord> recordsForMaterial, Material material) {

        int totalAmountToPayForMaterial = 0;

        for (InvoiceRecord invoiceRecord : recordsForMaterial) {
            totalAmountToPayForMaterial += calculateAmountToPayForRecord(invoiceRecord, material);
        }
    ...
    private static int calculateAmountToPayForRecord(InvoiceRecord invoiceRecord, Material material) {
        int amountOfTons = invoiceRecord.getAmountOfTons();
        ...
        // I calculate the amount to pay for this record
        int pricePerTonPerDay = material.getStoragePricePerTonPerDay();
        return amountOfTons * pricePerTonPerDay * daysInStorage;
    }
